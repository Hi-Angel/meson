...
external dependencies (including libraries) must go to "dependencies".''')
    ...
    @FeatureNewKwargs('dependency', '0.50.0', ['notfound_message'])
    @disablerIfNotFound
    ...
    def func_dependency(self, node, args, kwargs):
        ...
        notfound_message = kwargs.get('notfound_message', '')
        if not isinstance(notfound_message, str):
            raise InvalidArguments('The notfound_message must be a string.')
        try:
            d = self.dependency_impl(name, display_name, kwargs)
        except Exception:
            if notfound_message:
                self.message_impl(notfound_message)
            raise
        if not d.found() and notfound_message:
            self.message_impl(notfound_message)
        return d
